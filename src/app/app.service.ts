...
export class AppService {
  ...
  public getPropertiesByAgentId(agentId): Observable<Property[]> {
    return this.http.get<Property[]>(
      this.url + "properties-agentid-" + agentId + ".json"
    );
  }
  ...
  public addToCompare(property: Property, component, direction) {
    if (!this.Data.compareList.filter((item) => item.id == property.id)[0]) {
      ...
      this.stringCompare = localStorage.setItem(
        "compare",
        JSON.stringify(this.Data.compareList)
      );
      this.bottomSheet
        .open(component, {
          direction: direction,
  ...
  public addToFavorites(property: Property, direction) {
    if (!this.Data.favorites.filter((item) => item.id == property.id)[0]) {
      ...
      this.stringfavorites = localStorage.setItem(
        "favorites",
  ...
  public filterData(data, params: any, sort?, page?, perPage?) {
    if (params) {
      ...
      if (params.courtesy) {
        data = data.filter(
          (property) => property.courtesy == params.courtesy.name
        );
      }
      ...
      switch (sort) {
        ...
        case "Precio (Bajo a Alto)":
          if (this.appSettings.settings.currency == "USD") {
            data = data.sort((a, b) => {
              if (
                (a.priceDollar.sale || a.priceDollar.rent) >
          ...
          if (this.appSettings.settings.currency == "EUR") {
            data = data.sort((a, b) => {
              if (
                (a.priceEuro.sale || a.priceEuro.rent) >
