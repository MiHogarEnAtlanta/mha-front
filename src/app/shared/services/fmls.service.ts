...
export class FmlsService {
  ...
  cleanData(data: any) {
    ...
    data.forEach((element) => {
      ...
      if (element["ExteriorFeatures"].length != 0) {
        element["ExteriorFeatures"].forEach((item) =>
          this.dataArray["addFeatures"].push(item)
        );
      }
      if (element["CommunityFeatures"].length != 0) {
        element["CommunityFeatures"].forEach((item) =>
          this.dataArray["addFeatures"].push(item)
        );
      }
      if (element["AccessibilityFeatures"].length != 0) {
        element["AccessibilityFeatures"].forEach((item) =>
          this.dataArray["addFeatures"].push(item)
        );
      }
      ...
      if (element["Media"] != null) {
        element["Media"].forEach((item) =>
          this.dataArray["gallery"].push(item.MediaURL)
        );
      } else {
        this.dataArray["gallery"].push(
          "assets/images/others/NoImageAvailable.jpg"
        );
      }
      ...
      this.propertyN = new Property(
        ...
        this.dataArray["lastUpdated"],
        0,
        this.dataArray["courtesy"]
      );
  ...
  getListingKey(id) {
    return this.httpClient.get<any>(
      "https://api.bridgedataoutput.com/api/v2/fmls/listings/" +
        id +
  ...
  singleFmlsData(data: any) {
    ...
    if (data["ExteriorFeatures"].length != 0) {
      data["ExteriorFeatures"].forEach((item) =>
        this.dataArray["addFeatures"].push(item)
      );
    }
    if (data["CommunityFeatures"].length != 0) {
      data["CommunityFeatures"].forEach((item) =>
        this.dataArray["addFeatures"].push(item)
      );
    }
    if (data["AccessibilityFeatures"].length != 0) {
      data["AccessibilityFeatures"].forEach((item) =>
        this.dataArray["addFeatures"].push(item)
      );
    }
    this.dataArray["gallery"] = [];
